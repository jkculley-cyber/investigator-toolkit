          ...
          {CHECKLIST_ITEMS.map((item) => {
            ...
            return (
              <div key={item.field}>
                ...
                {/* Inline result prompt for manifestation determination */}
                {item.hasResult && isChecked && !checklist.manifestation_result && (
                  <div className="mx-3 mb-2 p-2.5 bg-amber-50 border border-amber-300 rounded-lg flex items-center gap-2">
                    <svg className="w-4 h-4 text-amber-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                    </svg>
                    <p className="text-xs text-amber-800 font-medium">
                      Record determination result above — IS or NOT manifestation must be selected to proceed.
                    </p>
                  </div>
                )}
